...
#cmt_local_tagfile_install_more_includes = $(MongoDBSvc_tag)_install_more_includes.make
cmt_local_tagfile_install_more_includes = $(bin)$(MongoDBSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(MongoDBSvc_tag).make
cmt_local_tagfile_install_more_includes = $(bin)$(MongoDBSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)MongoDBSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)MongoDBSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "MongDBSvc" = ""; then \
	    if test -d ../MongDBSvc; then \
	      echo "Installing files from more ../MongDBSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/MongDBSvc; then mkdir -p $(install_include_dir)/MongDBSvc; fi;\
	        $(install_command) ../MongDBSvc $(install_include_dir);\
	    else \
	      echo "No more MongDBSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/MongDBSvc"; \
	  $(uninstall_command) "$(install_include_dir)/MongDBSvc/*" ; \
	  if test -f $(install_include_dir)/MongDBSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/MongDBSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/MongDBSvc.cmtref ; \
	  fi; \
	fi
